...
2. 在 bgm.tv 页面的本地 NCM 版中点击“登录”，使用网易云音乐 App 扫码。登录成功后，本地服务会把登录态保存到 `~/.bangumi-plus/ncm-cookie.json`；之后即使关闭浏览器，只要本地服务还在运行，就可以继续使用账号权限。
...
- BGM 本地 NCM 版支持网易云音乐二维码登录；登录态由本地服务保存，播放时只使用账号实际拥有的权限。
...
| 网易云登录状态 | 支持 | 不支持 | 本地二维码登录 |
| 已购/会员歌曲试听 | 支持 | 不支持 | 支持账号已有权限 |
