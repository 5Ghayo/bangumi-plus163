...
(function () {
   ...
   */
  var require_scheduler_production = /* @__PURE__ */ __commonJSMin(
    (exports) => {
  ...
  async function resolveNeteaseAudioUrl({
    songId,
    endpoint = "/api/netease/song/enhance/player/url/v1",
    signal,
    requestJson,
  }) {
    ...
    if (requestJson) data = await requestJson(url.toString(), signal);
      ...
      if (!response.ok)
        throw new Error(`网易云音频加载失败（HTTP ${response.status}）`);
      data = await response.json();
    }
    if (data.code !== void 0 && data.code !== 200)
      throw new Error(`网易云音频加载失败（${data.code}）`);
    const source =
      data.data?.find((item) => item.id === songId) ?? data.data?.[0];
    if (!source?.url)
      throw new Error(
        source?.code === -110 ? "该曲目暂不可试听" : "该曲目没有可用的试听音源",
      );
  ...
  async function requestJson$1(url, signal, requester) {
    ...
    const response = await fetch(url, { signal });
    if (!response.ok)
      throw new Error(`网易云请求失败（HTTP ${response.status}）`);
  ...
  async function searchSongs(query, endpoint, signal, requester) {
    ...
    if (data.code !== void 0 && data.code !== 200)
      throw new Error(`网易云搜索失败（${data.code}）`);
        ...
        .catch((reason) => {
          if (!controller.signal.aborted)
            setSettled({
              ...
              result: null,
              error:
                reason instanceof Error ? reason.message : "网易云搜索失败",
            });
        });
  ...
  function findBangumiTrackListMountPoint() {
    return (
      [...document.querySelectorAll("h2.subtitle, h2, h3, .subtitle")].find(
        (element) =>
          element.textContent?.replace(/\s+/g, "").includes("曲目列表"),
      )?.parentElement ??
  ...
  function createBangumiTrackListShadowMount(options) {
    ...
    document
      .querySelectorAll("[data-bangumi-music-player]")
  ...
  function MusicPreviewBar({
    ...
    const rowBindings = (0, import_react.useSyncExternalStore)(
    ...
    (0, import_react.useEffect)(() => {
      if (
        !document.head.querySelector("[data-bangumi-plus-track-control-style]")
      ) {
    ...
    (0, import_react.useEffect)(() => {
      ...
      const updateTime = () =>
        setPlayer((current) =>
          current.songId === currentSongIdRef.current
            ? {
                ...current,
    ...
    const toggleSong = async (song) => {
      ...
      try {
        ...
        if (controller.signal.aborted || currentSongIdRef.current !== song.id)
          return;
        audio.src = url;
        audio.load();
        await audio.play();
            ...
            children: [
              ...
              !loading &&
                ...
                /* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", {
                  className:
                    "music-preview__status music-preview__status--error",
                  children: error,
                }),
              !loading &&
                !error &&
                ...
                /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("p", {
                  className: "music-preview__status",
                  children: ["没有找到匹配的", sourceLabel, "曲目"],
                }),
              !loading &&
                !error &&
                songs.length > 0 &&
  ...
  var GD_SONG_SEARCH_COUNT = 20;
  var GD_ALBUM_SEARCH_COUNT = 99;
  var GD_AUDIO_BITRATE = 128;
  ...
  async function requestGd(params, signal) {
    ...
    if (!response.ok)
      throw new Error(`GD 音乐台请求失败（HTTP ${response.status}）`);
    const data = await response.json();
    if (isErrorPayload(data))
      throw new Error(data.detail ?? "GD 音乐台请求失败");
    return data;
  }
  function parseAudioSongId(rawIds) {
    if (!rawIds) return NaN;
    try {
   ...
   */
  function createGdstudioRequester() {
    const albumIds = /* @__PURE__ */ new Map();
    const albumNames = /* @__PURE__ */ new Map();
    let nextAlbumId = 1;
    const albumIdFor = (albumName) => {
    ...
    const mapSong = (song) => ({
      ...
      album: {
        ...
        name: song.album,
        picUrl: song.pic_id
          ? `https://p1.music.126.net/${song.pic_id}.jpg`
          : void 0,
      },
    });
    const searchSongs = async (query, signal) => {
      return {
        code: 200,
        result: {
          songs: (
    ...
    const albumTracks = async (albumName, albumId, signal) => {
      const songs = await requestGd(
        new URLSearchParams({
          types: "search",
      ...
      return {
        ...
        album: {
          ...
          name: albumName,
          picUrl: first?.pic_id
            ? `https://p1.music.126.net/${first.pic_id}.jpg`
            : void 0,
          songs: songs.map(mapSong),
        },
      };
    };
    const songAudio = async (songId, signal) => {
      const url = (
        await requestGd(
          new URLSearchParams({
            types: "url",
  ...
  var requestJson = createGdstudioRequester();
  ...
  function isDarkBangumiTheme() {
    ...
    for (const element of [
      ...
      if (!element) continue;
      const luminance = parseRgbLuminance(
  ...
  function installBangumiThemeSync() {
    ...
    window
      ...
      .addEventListener("change", applyBangumiTheme);
  }
  function getPageSubject(subjectId) {
    const title =
      document
        .querySelector("#headerSubject h1, h1.nameSingle, h1")
        ?.textContent?.trim() ||
  ...
  async function mount() {
    ...
    try {
      ...
      (0, import_client.createRoot)(mount.root).render(
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)(MusicPreviewBar, {
          subject,
      ...
      window.setTimeout(() => {
        const toolbarButton = mount.root.querySelector(
          ".music-preview__toggle",
        );
      ...
      console.info("[bangumi-plus/bgm] preview button mounted");
    } catch (error) {
      console.warn("[bangumi-plus/bgm] 音乐播放器加载失败", error);
    }
  }
  console.info("[bangumi-plus/bgm] component started", location.href);
  mount();
  //#endregion
})();
